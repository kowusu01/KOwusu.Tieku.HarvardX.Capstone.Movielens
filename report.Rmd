...
```{r my_custom_themes}
    ...
    ## brewer.pal(7,"BrBG")
    ##
    # create my own color palete for long categorical list
...
``` {r cross_validation_results_chart, fig.height=3.5, fig.width=5}
...
```
A final model was fitted on the entire edx dataset to create the final user and movies biases using the optimized lambda of ``` final_model_info[1,2]`. The following tables show samples of the final user and movie biases.
...
The easiest way to execute the scriot is to clone the repo from github and use RStudio. When you close the repo, you will get the entire folder structure needed to execute the scripts.
